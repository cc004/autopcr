...
class travel_quest_sweep(Module):
    ...
    async def do_task(self, client: pcrclient):
        ...
        if any(self.can_receive_count(quest, client.time) for quest in top.travel_quest_list):
        ...
        if team_count and total_use: # avoid divide by zero
            ...
            speed_up_quest.sort(key=lambda x: self.quest_left_time(x, client.time), reverse=True) # large left time first
    ...
    async def do_task(self, client: pcrclient):
        ...
        for add_quest_id, (remove_quest_id, remove_quest) in zip(to_add, to_delete.items()):
            ...
            next_loop = next(i for i in range(remove_quest.received_count, remove_quest.total_lap_count + 1) 
                             ...
                             or remove_quest.travel_start_time + i * quest_interval - remove_quest.decrease_time > client.time)

            if next_loop < remove_quest.total_lap_count:
                delta_time = int(remove_quest.travel_start_time + next_loop * quest_interval - remove_quest.decrease_time - client.time)
            ...
            if self.is_finish_quest(remove_quest, client.time):
