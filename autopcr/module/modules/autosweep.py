...
@singlechoice("normal_sweep_consider_unit", "需求角色", "favorite",
              ["all", "max_rank", "max_rank-1", "max_rank-2", 'favorite'])
    ...
    async def do_task(self, client: pcrclient):
        ...
        try:
            for i in range(10000):
                ...
                if i % 3 == 0:
                    demand = Counter({token: require_equip[token] - client.data.get_inventory(token) for token in
                                      client.data._inventory if db.is_equip(token)})
