...
class Config():
    ...
    candidates: list
    config_type: str
    do_check: Callable[[pcrclient], Coroutine[Any, Any, Tuple[bool, str]]] # True for continue, False for stop
...
async def _do_check(client: pcrclient):
    pass
...
def config_option(key:str, desc: str, default, candidates: list = [], config_type='str', do_check = _do_check, check: bool = False):
    ...
    def wrapper(cls: Module):
        config = Config(key=key, desc=desc, default=default, candidates=candidates, config_type=config_type, do_check = do_check)
...
def conditional_execution(key: str, default, desc: str = "执行条件", check: bool = True):
    async def do_check(client: pcrclient) -> Tuple[bool, str]:
        run_time = set(client.keys.get(key, default))
        if not (
